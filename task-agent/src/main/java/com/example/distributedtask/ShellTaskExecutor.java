...
public class ShellTaskExecutor implements TaskExecutor {
    ...
    public String execute(String command) {
        ...
        try {
            ...
            if (!finished) {
                log.warn("Shell command timed out after 60 seconds: {}", command);
                process.destroyForcibly();
                return "Command timed out.\n" + output.toString();
            }

            int exitCode = process.exitValue();
            log.info("Shell command '{}' exited with code {}", command, exitCode);

            if (exitCode != 0) {
                log.error("Shell command failed with exit code {}. Output:\n{}", exitCode, output.toString());
                return "Command failed with exit code " + exitCode + ".\n" + output.toString();
            }

        } catch (Exception e) {
            log.error("Error executing shell command: {}", command, e);
